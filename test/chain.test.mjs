...
test('v0.1.23 sends natural prompt text without verification prefixes and reacquires animated model rows',async()=>{
...
test('v0.1.23 never sends or reload-recovers an extra Sol unlock turn',async()=>{
 const background=await r('extension/background.js');
...
test('v0.1.23 has one final model-row activation authority without hit-test retry layers',async()=>{
...
test('v0.1.23 manual verification owns a clean diagnostic session and publishes final Work state',async()=>{
 const background=await r('extension/background.js');
...
test('v0.1.23 packaged prompt bank remains intact without page Work-control dependency',async()=>{
...
test('v0.1.23 distinguishes inline A rows from the real Select-model B catalog',async()=>{
 const content=await r('extension/content.js');
...
test('v0.1.23 unlocks real picker B with one normal Work-policy turn after verified Sol',async()=>{
 const background=await r('extension/background.js');
...
test('v0.1.23 Work completion is owned by observed picker B, not runtime flag alone',async()=>{
 const background=await r('extension/background.js');
